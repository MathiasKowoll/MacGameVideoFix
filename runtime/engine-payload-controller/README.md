...
**Three PE files, five patches.** Built from the engine's own wine source — the
revision `built-for.json` records — with `mgvf-0002`, `mgvf-0003`, `mgvf-0004`,
`mgvf-0005` and `mgvf-0007` on top. `source-patches/README.md` says what each
one does.

**Two more patches exist and are deliberately not built.** `mgvf-0006` seizes a
Bluetooth DualSense from macOS so that wine is the only writer on the pad's
single output pipe. It does that, and the contention it was written for was
measured — but that contention is not what made the pad drop its link, and the
one thing a person sees from the patch is that macOS and its own applications
cannot use the pad while a bottle holds it, which arrives as a controller that
will not come back on. `mgvf-0008` was an **experiment**: it answered a feature
write in the emulation path without sending it, to ask whether that write was
what made the pad leave. It was run and it answered — with the write forwarded
again the pad did not die either — so the question is settled and the
instrument comes out. Both patch files stay in `source-patches/` as the record
of the work. `mgvf-0006` was also the only change to the unix half, which is
why this set is three PE files and ships no `winebus.so`: without that patch,
our build of it would be CodeWeavers' build of it.
...
app name is what tells them apart.
...
there, `broken` when some are, `absent` when none is. One more thing counts as
`broken`: a `winebus.so.mgvf-stock` in `lib/wine/x86_64-unix/`, which only the
version of this set that shipped a unix half can have left. That engine is
carrying our `winebus.so` beside three PE files that were not built with the
patch it exists for, so both `--restore` and a fresh install put CodeWeavers'
own file back, and the install does it before it writes anything else. Two
differences from the media installer. It refuses while a bottle is running —
`winedevice.exe` holds `winebus.sys` and `ntoskrnl.exe`, and a swap while it
runs leaves the bottle half on each set until it shuts down — in both
directions, so close Steam first. And it re-signs the bundle and clears its
quarantine attribute itself, in that order, because it runs after the engine
copy exists with no signing step after it.
...
counted. The **bytes** column is as built on 2026-09-09 and is for the reader;
