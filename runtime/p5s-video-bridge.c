/* p5s-video-bridge -- carry Persona 5 Strikers' video from D3D9 to D3D11.
 *
 * The game decodes VC-1 with Media Foundation, blits the frame into a D3D9
 * surface, and displays it from D3D11 through a shared handle. Wine cannot
 * share a D3D9 surface: CreateRenderTarget returns S_OK and a NULL handle --
 * it warns and continues -- so the game believes it succeeded and quietly
 * stops, never reading a single sample.
 *
 * Five things had to line up, and each was measured rather than reasoned:
 *
 * 1. A genuine shared handle, from a DXMT D3D11 texture. DXMT implements
 *    sharing where Wine does not, which is why this game is only fixable on
 *    that backend. Handing one back is what makes the game start reading.
 *
 * 2. The texture the game actually reads. It opens the handle on ITS OWN
 *    device, and writes to a texture on ours are not visible there -- a
 *    hundred frames went into something nobody was looking at while the screen
 *    stayed magenta. So OpenSharedResource is intercepted and answered with a
 *    texture created on the game's device.
 *
 * 3. That texture must look like a shared one: bindable as a render target as
 *    well as a shader resource, and carrying MISC_SHARED. Without it the run
 *    ended early, somewhere far from the cause.
 *
 * 4. ID3D11Multithread, which DXMT ships disabled. Turning it on is what makes
 *    writing from the video thread to the renderer's device legal, with
 *    ordinary D3D11 ordering against the engine's reads.
 *
 * 5. NV12 to BGRA, from the surface the game is blitting FROM. The destination
 *    cannot be read back -- GetRenderTargetData answers D3DERR_INVALIDCALL --
 *    and the source is NV12, which is why raw bytes produced noise.
 *
...
#define LOGFILE "C:\\p5s-video-bridge.log"
