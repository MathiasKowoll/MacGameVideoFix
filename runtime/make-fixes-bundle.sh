...
# Schema 4 adds "scope". A consumer that ignores it keeps working -- every
# entry that existed under 3 reads "folder", which is what it always was -- but
# a consumer that needs it has no way to tell 3 from 4 unless the number moves,
# and one already depends on it. Adding a field quietly is how a version number
