...
<plist version="1.0"><dict>
  ...
  <key>CFBundleVersion</key><string>5.0.5</string>
  <key>CFBundleShortVersionString</key><string>5.0.5</string>
...
#
# The glob is deliberately extension-blind, and the set keeps growing: ten files
# now. Nine are PE -- winebus.sys, setupapi.dll, ntoskrnl.exe, hidclass.sys and
# five xinput DLLs -- and engine-controller-winebus.so is the unix half of
# winebus, which the installer writes to lib/wine/x86_64-unix/ rather than to
# lib/wine/x86_64-windows/ with the others. Narrowing this to *.dll or *.sys
# would drop the unix half, and the set would install as PE files that no longer
# match it. Listing them by name would be worse: the list has changed three
# times and every copy of it is another place to forget.
